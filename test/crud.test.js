...
describe('SuperModel CRUD functionality', () => {
  beforeAll(() => {
  ...
  describe('Validate save', () => {
    ...
    it('should validate own attributes', () => {
      expect(specimen.validateSave().firstName).toBe('hello');
    });
    ...
    it('should not validate when  Model.validate is not present', async () => {
      const Model = SuperModel.extend({ tableName: 'crud_table' });
      const model = await Model.forge({ id: 1 }).save('firstName', 'notYoName');
